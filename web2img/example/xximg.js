var HASH = 'aYR60zuQwZAXdgR/scFKvuX7PpYVEF5kmh/tfQ0FYJM13'
var URLS = ['http://127.0.0.1:8003/example/web.png','http://127.0.0.1:8003/example/web1.png','http://127.0.0.1:8003/example/noweb.png']

var PRIVACY = 2
var UPDATE_INTERVAL = 60
var IMG_TIMEOUT = 10
var pathname=location.pathname.substring(0,location.pathname.lastIndexOf("/"));
var cacheKey='.web2img-'+pathname;
function pageEnv() {
  ...
  function parseImgBuf(buf) {
    ...
    crypto.subtle.digest('SHA-256', buf).then(function(digest) {
      ...
      // caches.delete(cacheKey).then(function() {
...
function swEnv() {
  ...
  function respondFile(req) {
    var path = new URL(req.url).pathname
      .replace(/\/{2,}/g, '/')
      .replace(/\/$/, '/index.html')

    return openFile(path).then(function(r1) {
        return r1  || fetch(req).then(responese => {
          const responeseClone = responese.clone();
          caches.open(cacheKey).then(cache => {
            cache.put(req, responeseClone);
          })
          return responese;
        })
        .catch(err => {
        ...
        || openFile(rootPath + '404.html').then(function(r2) {
  ...
  function respond(req) {
    return caches.has(cacheKey).then(function(existed) {
      if (!existed) {
        // fix cache
        newJs = jsUrl
      }
